...
function SQ_X(sq) {
  return (SQUARE_LEFT + (FILE_X(sq) - 3) * SQUARE_SIZE) + 'px';
}

function SQ_Y(sq) {
  return (SQUARE_TOP + (RANK_Y(sq) - 3) * SQUARE_SIZE) + 'px';
}
...
function alertDelay(message) {
  sendMovePauseClock('move ' + board.pos.mvList[board.pos.mvList.length-1] +' '+ toFen())
...
function Board(container, images, sounds) {
  ...
  for (var sq = 0; sq < 256; sq ++) {
    ...
    var style2 = img.style;
    style2.position = "absolute";
    style2.left = SQ_X(sq);
    style2.top = SQ_Y(sq);
    style2.width = SQUARE_SIZE + 'px';
    style2.height = SQUARE_SIZE + 'px';
    style2.zIndex = 100;
    ...
    var timer = setInterval(function() {
      if (step == 0) {
        ...
        this_.imgSquares[sqMate].src = this_.images +
            (this_.pos.sdPlayer == 0 ? "r" : "b") + "km2.png";
...
Board.prototype.drawSquare = function(sq, selected) {
  ...
  img.src = this.images + PIECE_NAME[this.pos.squares[sq]] + "2.png";
